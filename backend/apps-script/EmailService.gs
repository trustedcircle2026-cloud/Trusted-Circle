...
var TC_EMAIL={PRIMARY:'info@trustedcircle.in',INFO:'promoter@trustedcircle.in'};
...
function sendTransactionalEmail_(to,subject,htmlBody,textBody){if(!isValidEmail_(to))return false;try{MailApp.sendEmail({to:to,subject:subject,htmlBody:htmlBody,body:textBody,name:'Trusted Circle',replyTo:TC_EMAIL.INFO});return true;}catch(mailError){try{var configuredFrom=normalizeEmail_(getScriptProperties_().getProperty('EMAIL_FROM')||TC_EMAIL.PRIMARY),from=TC_EMAIL.PRIMARY,options={htmlBody:htmlBody,body:textBody,name:'Trusted Circle',replyTo:TC_EMAIL.INFO},aliases=emailAliasList_();if(aliases.indexOf(from)<0&&aliases.indexOf(configuredFrom)>=0)from=configuredFrom;if(aliases.indexOf(from)<0)throw new Error('Sender email '+from+' is not configured as a Gmail alias for the Apps Script account.');options.from=from;GmailApp.sendEmail(to,subject,textBody,options);return true;}catch(gmailError){console.error('Transactional email failed. MailApp: '+mailError.message+' | GmailApp: '+gmailError.message);return false;}}}
