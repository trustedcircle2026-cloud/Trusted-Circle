...
function sendTransactionalEmail_(to,subject,htmlBody,textBody){if(!isValidEmail_(to))return false;var from=normalizeEmail_(getScriptProperties_().getProperty('EMAIL_FROM')||TC_EMAIL.PRIMARY),options={htmlBody:htmlBody,body:textBody,name:'Trusted Circle',replyTo:TC_EMAIL.INFO},aliases=emailAliasList_();if(aliases.indexOf(from)>=0)options.from=from;try{GmailApp.sendEmail(to,subject,textBody,options);return true;}catch(e){try{MailApp.sendEmail({to:to,subject:subject,htmlBody:htmlBody,body:textBody,name:'Trusted Circle',replyTo:TC_EMAIL.INFO});return true;}catch(ignore){console.error('Transactional email failed: '+e.message);return false;}}}
