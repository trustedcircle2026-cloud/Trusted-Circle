...
var TC_EMAIL={PRIMARY:'trustedcircle2026@gmail.com',INFO:'promoter@trustedcircle.in'};
...
function sendTransactionalEmail_(to,subject,htmlBody,textBody){if(!isValidEmail_(to))return false;try{MailApp.sendEmail({to:to,subject:subject,htmlBody:htmlBody,body:textBody,name:'Trusted Circle',replyTo:TC_EMAIL.INFO});return true;}catch(mailError){try{var from=normalizeEmail_(getScriptProperties_().getProperty('EMAIL_FROM')||TC_EMAIL.PRIMARY),options={htmlBody:htmlBody,body:textBody,name:'Trusted Circle',replyTo:TC_EMAIL.INFO},aliases=emailAliasList_();if(aliases.indexOf(from)>=0)options.from=from;GmailApp.sendEmail(to,subject,textBody,options);return true;}catch(gmailError){console.error('Transactional email failed. MailApp: '+mailError.message+' | GmailApp: '+gmailError.message);return false;}}}
