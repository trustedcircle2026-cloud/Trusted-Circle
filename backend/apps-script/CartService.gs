...
var CartService = {
  ...
  addItem: function(data) {
    ...
    require_(
      productId && isFinite(quantity) && Math.floor(quantity) === quantity && quantity >= 1 && quantity <= 20,
      'Valid productId and integer quantity are required.'
    );

    var product = findOne_(TC_CONFIG.SHEETS.PRODUCTS, 'ProductID', productId);
    if (!product || String(product.Active).toUpperCase() === 'FALSE') {
      throw new Error('Product unavailable.');
    }
    if (String(product.InventoryStatus || 'AVAILABLE').toUpperCase() === 'OUT_OF_STOCK') {
      throw new Error('Product is out of stock.');
    }

    var rows = getRows_(TC_CONFIG.SHEETS.CART);
    var existing = rows.find(function(r) {
      return String(r.UserID) === String(user.UserID) && String(r.ProductID) === productId;
    });

    if (existing) {
      var next = Number(existing.Quantity || 0) + quantity;
      require_(next <= 20, 'Maximum quantity per product is 20.');
      updateRowById_(TC_CONFIG.SHEETS.CART, 'CartID', existing.CartID, {
        Quantity: next,
        UpdatedAt: isoNow_()
      });
    } else {
  ...
  updateItem: function(data) {
    ...
    updateRowById_(TC_CONFIG.SHEETS.CART, 'CartID', itemId, {
      Quantity: quantity,
      UpdatedAt: isoNow_()
    });
