...
function adminAddPaymentLinkStockBulk_(data){
  ...
  ensureColumns_(TC_CONFIG.SHEETS.PAYMENT_LINK_STOCK,['ProviderLinkID']);
  ...
  try{
    ...
    rows.forEach(function(raw,index){
      var denomination=Number(raw.denomination||raw.Denomination),link=cleanText_(raw.link||raw.Link||'',2000),label=cleanText_(raw.label||raw.Label||'Pay securely',100),providerLinkId=cleanText_(raw.providerLinkId||raw.ProviderLinkID||'',200);
      ...
      var id=newId_('TCPLS');
      appendRowObject_(TC_CONFIG.SHEETS.PAYMENT_LINK_STOCK,{PaymentLinkStockID:id,Denomination:denomination,Link:link,Label:label,Status:'AVAILABLE',OrderID:'',PaymentLinkID:'',ProviderLinkID:providerLinkId,CreatedAt:now,UpdatedAt:now});
