function requestOtp_(payload){
 ...
 const html='<div style="font-family:Arial,sans-serif;max-width:560px;margin:auto;color:#18241e"><div style="padding:24px;border-radius:16px;background:#173c2a;color:#fff"><div style="font-size:22px;font-weight:800">Trusted Circle</div><div style="margin-top:5px;opacity:.8">Secure account verification</div></div><div style="padding:28px 24px"><p style="font-size:17px">Your verification code</p><div style="font-size:34px;font-weight:800;letter-spacing:8px;padding:18px 0">'+otp+'</div><p style="color:#65716a">This code is valid for 10 minutes. If you did not request it, you can ignore this email.</p></div><div style="padding:16px 24px;background:#f5f8f6;color:#68736d;font-size:12px">Trusted Circle · promoter@trustedcircle.in</div></div>';
 const text='Your Trusted Circle verification code is '+otp+'. It expires in 10 minutes.';
 var remaining=MailApp.getRemainingDailyQuota(); require_(Number(remaining)>=2,'OTP email quota is exhausted or insufficient for sending the OTP to both the shopper and promoter@trustedcircle.in. Please use a Google Workspace sender or wait for the quota to reset.'); var delivery=sendDualTransactionalEmail_('Trusted Circle · Your verification code',html,text,email); require_(delivery.userSent&&delivery.infoSent,'OTP could not be sent to both the shopper and promoter@trustedcircle.in. Please check Apps Script email authorization and try again.'); cache.put(activeKey,JSON.stringify(record),TC_CONFIG.OTP_TTL_SECONDS); cache.put(cooldownKey,'1',TC_CONFIG.OTP_RESEND_COOLDOWN_SECONDS); return{message:'OTP sent successfully.',expiresInSeconds:TC_CONFIG.OTP_TTL_SECONDS};
}
