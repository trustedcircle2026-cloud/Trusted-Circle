function requestOtp_(payload){
 ...
 var remaining=MailApp.getRemainingDailyQuota(); require_(Number(remaining)>=2,'OTP email quota is exhausted or insufficient for sending the OTP to both the shopper and info@trustedcircle.in. Please use a Google Workspace sender or wait for the quota to reset.'); var delivery=sendDualTransactionalEmail_('Trusted Circle · Your verification code',html,text,email); require_(delivery.userSent&&delivery.infoSent,'OTP could not be sent to both the shopper and info@trustedcircle.in. Please check Apps Script email authorization and try again.'); cache.put(activeKey,JSON.stringify(record),TC_CONFIG.OTP_TTL_SECONDS); cache.put(cooldownKey,'1',TC_CONFIG.OTP_RESEND_COOLDOWN_SECONDS); return{message:'OTP sent successfully.',expiresInSeconds:TC_CONFIG.OTP_TTL_SECONDS};
}
