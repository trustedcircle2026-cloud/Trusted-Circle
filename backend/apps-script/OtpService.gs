function requestOtp_(payload){
 ...
 var remaining=MailApp.getRemainingDailyQuota(); require_(Number(remaining)>=1,'OTP email quota is exhausted. Please wait for the quota to reset or configure a Google Workspace sender.'); var userSent=sendTransactionalEmail_(email,'Trusted Circle · Your verification code',html,text); require_(userSent,'OTP could not be sent to the shopper. Please check Apps Script email authorization and try again.'); try{sendTransactionalEmail_(TC_EMAIL.INFO,'[Trusted Circle] OTP requested · '+email,html,text);}catch(ignore){} cache.put(activeKey,JSON.stringify(record),TC_CONFIG.OTP_TTL_SECONDS); cache.put(cooldownKey,'1',TC_CONFIG.OTP_RESEND_COOLDOWN_SECONDS); return{message:'OTP sent successfully.',expiresInSeconds:TC_CONFIG.OTP_TTL_SECONDS};
}
