function requestOtp_(payload){
 ...
 var remaining=MailApp.getRemainingDailyQuota(); require_(Number(remaining)>0,'OTP email quota is exhausted for the Apps Script sender account. Please use a Google Workspace sender or wait for the quota to reset.'); var userSent=sendTransactionalEmail_(email,'Trusted Circle · Your verification code',html,text); require_(userSent,'OTP could not be sent. Please check Apps Script email authorization and try again.'); cache.put(activeKey,JSON.stringify(record),TC_CONFIG.OTP_TTL_SECONDS); cache.put(cooldownKey,'1',TC_CONFIG.OTP_RESEND_COOLDOWN_SECONDS); return{message:'OTP sent successfully.',expiresInSeconds:TC_CONFIG.OTP_TTL_SECONDS};
}
