...
function adminWorklistAction_(data){
  ...
  if(action==='REMOVE'){
    ...
    appendAudit_(admin.UserID,'WORKLIST_REMOVE',sheet,recordId,{reason:reason||'Removed by admin'});return{removed:true};
  }
  ...
  if(action==='VERIFY_PAYMENT'){
    ...
    var orderId=row.OrderID,payment=sheet==='Payments'?row:((getRows_(TC_CONFIG.SHEETS.PAYMENTS).filter(function(p){return String(p.OrderID)===String(orderId);}).slice(-1)[0]||null));
    require_(payment,'Payment record not found.');var expected=Number(sheet==='Payments'?row.Amount:row.Total),paid=Number(payment.Amount);require_(expected===paid,'Payment amount does not match the order amount.');
    updateRowById_(TC_CONFIG.SHEETS.PAYMENTS,'PaymentID',payment.PaymentID,{Status:'VERIFIED',VerifiedAt:isoNow_(),UpdatedAt:isoNow_()});updateRowById_(TC_CONFIG.SHEETS.ORDERS,'OrderID',orderId,{Status:'PAID',UpdatedAt:isoNow_()});
    ...
    adminClearRowsCache_('Payments');adminClearRowsCache_('Orders');adminClearRowsCache_('CashbackWallet');adminClearRowsCache_('CashbackTransactions');adminClearRowsCache_('PaymentLinkStock');appendAudit_(admin.UserID,'PAYMENT_VERIFIED','Order',orderId,{paymentId:payment.PaymentID,amount:paid,reason:reason||''});return{payment:findOne_(TC_CONFIG.SHEETS.PAYMENTS,'PaymentID',payment.PaymentID),order:findOne_(TC_CONFIG.SHEETS.ORDERS,'OrderID',orderId)};
  }
  ...
  if(action==='REJECT'){var updated=adminSetStatus_(sheet,idField,recordId,'REJECTED',admin,reason);if(sheet==='WalletRedemptions'&&typeof restoreRejectedRedemption_==='function')restoreRejectedRedemption_(updated);return{row:updated};}
