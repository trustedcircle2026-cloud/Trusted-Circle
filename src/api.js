...
async function request(action, params = {}, method = 'GET') {
  ...
  if (method === 'GET') {
    ...
    response = await fetch(API_BASE, { method: 'POST', headers: { 'Content-Type': 'text/plain;charset=UTF-8' }, body: JSON.stringify(payload), credentials: 'omit' })
  }
...
export const api = {
  ...
  profileUpdate: (token, name) => request('profileUpdate', { token, name }, 'POST'),
  ...
  placeOrder: (token) => request('placeOrder', { token }, 'POST'),
