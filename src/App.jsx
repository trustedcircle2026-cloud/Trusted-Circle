...
function App() {
  ...
  return (
    <div className="app-shell">
      ...
      <main>
        <section className="hero" id="home">
          ...
          <div className="hero-card-wrap" aria-hidden="true">
            ...
            <div className="floating-card card-main"><div className="card-top"><span className="card-brand"><img src={LOGO_URL} alt="" /> TRUSTED CIRCLE</span><Gift size={27} /></div><div className="card-value">₹1,000</div><div className="card-bottom"><span>Digital Gift Voucher</span><span>•••• 2026</span></div></div>
      ...
      {authOpen && <div className="modal-backdrop" onMouseDown={e => e.target === e.currentTarget && setAuthOpen(false)}><div className="auth-modal"><button className="modal-close" onClick={() => setAuthOpen(false)} aria-label="Close"><X size={20} /></button><span className="modal-icon"><img src={LOGO_URL} alt="Trusted Circle" /></span><h2>{authStep === 'email' ? (authMode === 'register' ? 'Create your account' : 'Welcome back') : 'Enter your OTP'}</h2><p>{authStep === 'email' ? 'Use your email to receive a secure verification code.' : `We sent a 6-digit code to ${email}.`}</p>{authStep === 'email' ? <form onSubmit={requestOtp}>{authMode === 'register' && <input value={name} onChange={e => setName(e.target.value)} placeholder="Full name" maxLength={120} required />}<input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="Email address" autoFocus required /><button className="modal-submit" disabled={authBusy}>{authBusy ? 'Sending…' : 'Send OTP'}</button></form> : <form onSubmit={verifyOtp}><input inputMode="numeric" autoFocus value={otp} onChange={e => setOtp(e.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="6-digit OTP" maxLength={6} required /><button className="modal-submit" disabled={authBusy || otp.length !== 6}>{authBusy ? 'Verifying…' : 'Verify & Continue'}</button><button type="button" className="modal-secondary" onClick={() => setAuthStep('email')}>Change email</button></form>}{authMessage && <div className="auth-message">{authMessage}</div>}<div className="auth-switch">{authMode === 'login' ? <>New to Trusted Circle? <button onClick={() => { setAuthMode('register'); setAuthStep('email'); setAuthMessage('') }}>Create account</button></> : <>Already have an account? <button onClick={() => { setAuthMode('login'); setAuthStep('email'); setAuthMessage('') }}>Login</button></>}</div></div></div>}
    </div>
  )
}
