...
export default function App(){
 ...
 const verifyOtp=async event=>{event.preventDefault();if(otp.length!==6)return;setAuthBusy(true);setAuthMessage('Verifying…');try{const data=await api.verifyOtp(email,otp,'');localStorage.setItem('tc_session',data.session.token);setToken(data.session.token);setUser(data.user);setProfileName(data.user.name||'');const pending=guestCart;for(const item of pending){await api.cartAdd(data.session.token,item.productId,item.quantity,item.denomination)}localStorage.removeItem('tc_guest_cart');setGuestCart([]);await loadCart(data.session.token);setAuthOpen(false);const destination=postLoginRoute||'home';if(!data.user.name)setProfileOpen(true);navigate(destination)}catch(error){setAuthMessage(friendlyApiError(error))}finally{setAuthBusy(false)} }
 ...
 {route.path==='orders'&&<OrdersPage orders={orders} loading={ordersLoading} onBack={()=>navigate('vouchers')}/>}
 ...
 {authOpen&&<div className="modal-layer"><div className="auth-page-modal"><button className="modal-x" onClick={()=>setAuthOpen(false)} aria-label="Close login"><X size={17}/></button><span className="modal-logo-white"><img src={LOGO_URL} alt="Trusted Circle"/></span>{authStep==='email'?<><span className="eyebrow">ACCOUNT ACCESS</span><h2>Welcome to Trusted Circle</h2><p>Enter your email to continue.</p><form onSubmit={requestOtp}><input type="email" required value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@example.com" autoComplete="email"/><button className="btn-primary wide" type="submit" disabled={authBusy}>{authBusy?<><span className="auth-button-spinner"/>Sending code…</>:<>Continue <ArrowRight size={15}/></>}</button></form></>:<><span className="eyebrow">VERIFY EMAIL</span><h2>Enter your OTP</h2><p>We sent a code to <strong>{email}</strong>.</p><form onSubmit={verifyOtp}><input inputMode="numeric" autoComplete="one-time-code" autoFocus pattern="[0-9]{6}" maxLength="6" required value={otp} onChange={e=>setOtp(e.target.value.replace(/\D/g,''))} placeholder="6-digit OTP"/><button className="btn-primary wide" type="submit" disabled={authBusy||otp.length!==6}>{authBusy?<><span className="auth-button-spinner"/>Verifying…</>:<>Verify & continue <ArrowRight size={15}/></>}</button><button className="btn-quiet wide" type="button" disabled={authBusy} onClick={()=>setAuthStep('email')}>Use another email</button></form></>}{authBusy&&<div className="auth-fast-loading" role="status"><div className="auth-loading-logo"><img src={LOGO_URL} alt=""/></div><span>{authStep==='email'?'Sending code…':'Verifying your email…'}</span></div>}{authMessage&&<div className="form-message" role="alert">{authMessage}</div>}</div></div>}
